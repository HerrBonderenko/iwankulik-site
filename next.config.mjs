...
//
// Ціна компромісу (аудит, SECURITY_AUDIT_REPORT.md F-08): через
// 'unsafe-inline' політика НЕ зупинить вбудовані обробники
// onerror/onload, якщо десь пропущено екранування — другим рубежем
// проти XSS ця CSP не є. Поява будь-якого місця з користувацьким HTML
// (віджет, відгуки) вимагає спершу перейти на nonce, тобто на
