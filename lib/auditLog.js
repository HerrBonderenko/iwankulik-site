...
export async function logEvent({ action, user, ip, ua, detail }) {
  try {
    ...
    list.push({
      ...
      user: user || null,
      ip: ip || null,
      // User-Agent — єдине поле запису, яке ніхто не обмежував: воно
      // приходить заголовком, тобто будь-якої довжини, і осідає в сховищі
      // назавжди. 256 символів вистачає на будь-який справжній браузер.
