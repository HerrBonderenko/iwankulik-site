...
const FS_DATA = path.join(CONTENT_DIR, "data.json");
...
// cache() з React дедуплікує виклики в межах ОДНОГО рендера. Раніше
// getData() читав сховище на кожен виклик, а викликів на сторінку кілька:
// layout + сама сторінка, а на статті блогу — дев'ять (getPostMeta двічі,
// getAllPosts, і кожна з них тягне getBlogCovers + getBlogCoverAlts).
// Дев'ять — не з читання коду, а виміряно: лічильник у readStored() на
// повній збірці, 1177 читань на 167 сторінок проти 301 після правки.
// Читання йде з consistency: "strong", тобто повз CDN — по одному
// мережевому запиту на кожен виклик.
//
// Між рендерами кеш не живе: revalidate = 60 працює як і працював, дані
// лишаються свіжими. Дедуплікується тільки те, що всередині одного
// запиту й так мало б читатися один раз.
//
// Читання після запису в межах одного запиту ніде нема (перевірено:
// app/api/admin/data POST робить getData() до putData(), не після),
// тож віддати застаріле значення цей кеш не може.
...
export async function saveImage(buffer, filename, contentType, meta = {}) {
  ...
  const dir = path.join(CONTENT_DIR, "uploads");
