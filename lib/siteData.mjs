...
// blogCoverAlts — alt-описи обкладинок: slug → {локаль: рядок}. Двоповерхова
// мапа, тож перевіряємо обидва рівні: інакше сюди міг би заїхати об'єкт
// замість тексту й потрапити прямо в атрибут alt.
function isValidBlogCoverAlts(value) {
  if (value === undefined) return true;
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  return Object.values(value).every(
    (perLocale) =>
      perLocale &&
      typeof perLocale === "object" &&
      !Array.isArray(perLocale) &&
      Object.values(perLocale).every((v) => typeof v === "string")
  );
}

// Порожнє значення валідне (посилання можна прибрати); непорожнє мусить
// бути абсолютним http(s)-URL. Без import — модуль читає й голий node
// (див. шапку файлу), а URL є глобальним і там, і в Next.
...
export function validateSiteData(data) {
  const checks = [
    ...
    // Посилання йде прямо в href на кожній сторінці. React блокує лише
    // javascript:, data: тримається на політиці браузера — а тут
    // пускаємо тільки http(s), щоб не залежати від жодного з них
    // (аудит, R-04).
    ["contacts.instagram", () => isHttpUrl(data.contacts.instagram)],
    ["blogCovers", () => isValidBlogCovers(data.blogCovers)],
    ["blogCoverAlts", () => isValidBlogCoverAlts(data.blogCoverAlts)],
  ];
