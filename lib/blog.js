...
export function getPostSlugs(locale) {
  ...
  // Порожній масив тут повертається мовчки — саме це й треба побачити в лозі.
  if (!fs.existsSync(dir)) {
    console.log(`[build-diag] getPostSlugs(${locale}): КАТАЛОГУ НЕМА — ${dir}`);
    return [];
  }
  const slugs = fs
    .readdirSync(dir)
    ...
    .map((f) => f.replace(/\.mdx$/, ""));
  console.log(`[build-diag] getPostSlugs(${locale}): ${dir} → ${slugs.length} .mdx`);
  return slugs;
}
