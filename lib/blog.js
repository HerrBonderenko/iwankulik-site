...
// Той самий формат слага, який lib/blogValidation.js вимагає від файлів
// статей. locale і slug приходять з адреси сторінки — перевіряємо ДО
// path.join, інакше закодований роздільник у сегменті вивів би читання
// за межі blog-content (аудит, F-09).
const SLUG_RE = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;

function isValidPostRef(locale, slug) {
  return locales.includes(locale) && SLUG_RE.test(String(slug));
}

export function getPostSlugs(locale) {
  // locale теж буває з адреси (індекс блогу) — чужий рядок не пускаємо
  // в path.join, щоб не лістити теки поза blog-content.
...
function readFrontmatter(locale, slug) {
  if (!isValidPostRef(locale, slug)) return null;
