...
// Правило: поле, яке читає роут заявки, мусить бути в цьому списку —
// інакше воно обмежене лише розміром тіла (64 КБ) і їде в тему листа
// та в журнал як є (аудит, F-03).
const LENGTH_LIMITS = {
...
function signFormToken(ts, nonce) {
...
function checkFormTiming(ts, sig, nonce) {
  if (!ts || !sig || !nonce) return "missing";
  const expected = signFormToken(ts, nonce);
...
async function checkInquiryRateLimit(ip) {
  if (!ip) return true;
...
export async function guardInquiry({ data, ip, ua }) {
  ...
  // Сюди доходимо лише з валідним підписом, а він неможливий без nonce
  // (checkFormTiming вимагає всі три частини токена) — тож nonce тут є.
  ...
  const nonce = String(data.formNonce);
