...
import { isAllowedImageHost } from "@/lib/safeUrl";
...
async function readImageSource(src) {
  // src — значення з даних адмінки (img роботи/циклу, обкладинка).
  // Абсолютні адреси пускаємо лише в сховище Vercel Blob і на свій
  // домен: інакше сервер ходив би по довільному URL із поля (аудит, F-06).
  if (/^https?:\/\//.test(src)) {
    if (!isAllowedImageHost(new URL(src).host)) return null;
    return fetchImage(src);
  }
  if (src.startsWith("/uploads/")) {
    const name = path.basename(src);
    if (IS_NETLIFY) return getUploadedImage(name);
    return readFile(path.join(CONTENT_DIR, "uploads", name));
  }
  // Відносний шлях нормалізуємо й перевіряємо, що читання не вийшло за
  // межі public/: "../" у значенні поля вело б до файлів функції.
  const publicDir = path.join(process.cwd(), "public");
  const resolved = path.normalize(path.join(publicDir, src));
